...
from boukman import get_path_from_matrix
import numpy as np
...
def test_simple_graph():
    ...
    assert get_path_from_matrix(matrix=mm.matrix, source=mapper[A], target=mapper[C]) == [mapper[A], mapper[B], mapper[C]]
...
def test_take_longer_path_no_direct_edge():
    ...
    mm = mu.MappedMatrix(packages=[dp], matrix="test")
    mapper = mm.col_mapper.to_dict()
    assert get_path_from_matrix(matrix=mm.matrix, source=mapper[A], target=mapper[D]) == [mapper[A], mapper[B], mapper[C], mapper[D]]
